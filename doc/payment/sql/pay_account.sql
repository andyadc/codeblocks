...
CREATE TABLE `pay_account_0` (
  ...
  COMMENT '账户类型',
  `account_title` INT(12)             NOT NULL DEFAULT '0'
  COMMENT '会计科目代码',
  `fee_unit`      TINYINT(4)          NOT NULL DEFAULT '1'
  COMMENT '货币类型 1、人民币 2、积分 3、代金券 4、美元 5、台币',
  `third_type`    INT(8)              NOT NULL DEFAULT '0'
  COMMENT '第三方渠道ID，pay_partner表维护',
  `third_account` VARCHAR(32)         NOT NULL DEFAULT ''
  COMMENT '第三方的用户账户',
  `third_param`   VARCHAR(1024)       NOT NULL DEFAULT ''
  COMMENT '第三方凭证信息，通用字段，每个渠道区别维护',
