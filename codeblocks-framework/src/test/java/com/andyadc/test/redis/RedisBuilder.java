 ...
 * https://gist.github.com/JonCole/925630df72be1351b21440625ff2671f#file-redis-samplecode-java-jedispool-java
 */
public class RedisBuilder {
	...
	public static void initializeSettings(String host, int port, String password, int connectTimeout, int operationTimeout) {
		RedisBuilder.host = host;
		RedisBuilder.port = port;
		RedisBuilder.password = password;
		RedisBuilder.connectTimeout = connectTimeout;
		RedisBuilder.operationTimeout = operationTimeout;
	}
...
//			poolConfig.setMaxWaitMillis(operationTimeout);
			poolConfig.setMaxWait(Duration.ofMillis(operationTimeout));
			...
			RedisBuilder.config = poolConfig;
		}
		return config;
	}
	...
	private static class SimpleHostNameVerifier implements HostnameVerifier {
		...
		private final String exactCN;
		private final String wildCardCN;
